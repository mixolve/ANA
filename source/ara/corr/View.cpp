...
void CorrView::paint(juce::Graphics& graphics)
{
    ...
    if (const auto analysis = processor.getAraAnalysisResult();
        analysis != nullptr && analysis->corr != nullptr)
    {
        displayedCorr = analysis->corr.get();
        sampleRate = analysis->corrSampleRate;
    }
...
void CorrView::setCorrMode(const int mode)
{
    ...
    processor.requestAraAnalysis(
        static_cast<size_t>(std::max(1.0f, getPlotBounds().getWidth())), true);
}
