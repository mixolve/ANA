...
public:
    ...
    // Ara MAP accepts a display-raster-derived hop instead of an FFT-derived hop.
    template <typename FrameHandler>
    void processSpectrogramBlockWithHop(const juce::AudioBuffer<float>& buffer,
                                        ...
                                        FrameHandler&& onFrame) noexcept
    {
        ...
        // Preserve the ring across ARA blocks; analysis-state changes restart frame progress only.
        if (activeFftIndex != fftIndex || activeWindowType != WindowType::gaussian200Db)
        {
            activeFftIndex = fftIndex;
            activeHopSize = hopSize;
            activeWindowType = WindowType::gaussian200Db;
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        {
            ...
            validSampleCount = std::min(fftSize, validSampleCount + 1);

            if (++samplesSinceAnalysis >= hopSize)
            {
                samplesSinceAnalysis = 0;
                analyse(fftIndex, hopSize, WindowType::gaussian200Db, sample + 1,
                        std::forward<FrameHandler>(onFrame));
            }
        }
    }
...
private:
                 ...
                 const int sampleOffset, FrameHandler&& onFrame) noexcept
    {
        ...
        const auto& window = windowType == WindowType::gaussian200Db
            ? gaussian200Windows[windowIndex]
            : windowType == WindowType::gaussian800Db
                ? gaussian800Windows[windowIndex] : hannWindows[windowIndex];
        const auto coherentGain = windowType == WindowType::gaussian200Db
            ? gaussian200CoherentGains[windowIndex]
            : windowType == WindowType::gaussian800Db
                ? gaussian800CoherentGains[windowIndex] : hannCoherentGains[windowIndex];
    ...
    std::array<std::vector<float>, supportedFftSizes.size()> gaussian800Windows;
    std::array<float, supportedFftSizes.size()> hannCoherentGains {};
    std::array<float, supportedFftSizes.size()> gaussian200CoherentGains {};
    std::array<float, supportedFftSizes.size()> gaussian800CoherentGains {};
