...
constexpr float integratedLoudnessBinLow = -70.0f;
constexpr float integratedLoudnessBinStep = 0.1f;
...
{
    return juce::Decibels::gainToDecibels(std::max(gain, 1.0e-6f), floorDecibels);
}

float powerDecibels(const float power) noexcept
{
    return power > 1.0e-12f ? 10.0f * std::log10(power) : floorDecibels;
}

float lufsFromPower(const float power) noexcept
{
    return juce::jmax(floorDecibels, -0.691f + powerDecibels(power));
}

void configureHighShelf(MeterProcessor::Biquad& filter,
...
void configureHighPass(MeterProcessor::Biquad& filter,
                       const double sampleRate,
                       const double frequency,
...
float MeterProcessor::Biquad::process(const float input) noexcept
{
...
void MeterProcessor::prepare(const double newSampleRate, const int maximumBlockSamples) noexcept
{
    ...
    truePeakOversampler->initProcessing(static_cast<size_t>(std::max(1, maximumBlockSamples)));
    configureKWeighting();
    reset();
}
...
void MeterProcessor::reset() noexcept
{
    ...
    for (auto& value : publishedRmsMaximumDecibels)
        value.store(floorDecibels, std::memory_order_relaxed);
    publishedMomentaryLufs.store(floorDecibels, std::memory_order_relaxed);
    publishedShortTermLufs.store(floorDecibels, std::memory_order_relaxed);
    publishedIntegratedLufs.store(floorDecibels, std::memory_order_relaxed);
    publishedMomentaryMaximumLufs.store(floorDecibels, std::memory_order_relaxed);
    publishedShortTermMaximumLufs.store(floorDecibels, std::memory_order_relaxed);
    publishedIntegratedMaximumLufs.store(floorDecibels, std::memory_order_relaxed);
    ...
    pendingLoudnessFrameEnergy = 0.0;
    pendingLoudnessFrameSamples = 0;
    revision.fetch_add(1, std::memory_order_release);
}

void MeterProcessor::requestClear() noexcept
{
    clearRequested.store(true, std::memory_order_release);

    // Reset the values shown by the editor immediately, even while transport is stopped.
    for (auto& value : publishedPeakDecibels)
        value.store(floorDecibels, std::memory_order_release);
    for (auto& value : publishedRmsDecibels)
        value.store(floorDecibels, std::memory_order_release);
    for (auto& value : publishedPeakMaximumDecibels)
        value.store(floorDecibels, std::memory_order_release);
    ...
    for (auto& value : publishedRmsMaximumDecibels)
        value.store(floorDecibels, std::memory_order_release);
    publishedMomentaryLufs.store(floorDecibels, std::memory_order_release);
    publishedShortTermLufs.store(floorDecibels, std::memory_order_release);
    publishedIntegratedLufs.store(floorDecibels, std::memory_order_release);
    publishedMomentaryMaximumLufs.store(floorDecibels, std::memory_order_release);
    publishedShortTermMaximumLufs.store(floorDecibels, std::memory_order_release);
    publishedIntegratedMaximumLufs.store(floorDecibels, std::memory_order_release);
    publishedLoudnessRange.store(0.0f, std::memory_order_release);
    historyWriteCount.store(0, std::memory_order_release);
    revision.fetch_add(1, std::memory_order_release);
}

void MeterProcessor::setFrozen(const bool shouldFreeze) noexcept
{
    frozen.store(shouldFreeze, std::memory_order_release);
}

bool MeterProcessor::isFrozen() const noexcept
{
    return frozen.load(std::memory_order_acquire);
}

void MeterProcessor::processBlock(const juce::AudioBuffer<float>& buffer,
                                       ...
                                       const ProcessingOptions options) noexcept
{
    ...
    const auto processPeakSignal = options.peakRms || options.history;
    ...
    if (processPeakSignal && truePeakOversampler != nullptr)
    {
        ...
        const auto oversampled = truePeakOversampler->processSamplesUp(inputBlock);
        for (int channel = 0; channel < channelCount; ++channel)
        {
            ...
            blockPeaks[static_cast<size_t>(channel)] = std::max(std::abs(range.getStart()),
    ...
    const auto requestedRmsWindowSamples = static_cast<size_t>(juce::jlimit(
        ...
        juce::roundToInt(sampleRate * juce::jlimit(10.0f, 3000.0f, rmsWindowMilliseconds) * 0.001f)));
    ...
    for (int sample = 0; sample < sampleCount; ++sample)
    {
        ...
        const std::array<float, 4> meterSamples {
            left, right,
        ...
        if (options.peakRms)
        {
            ...
            for (size_t index = 0; index < meterSamples.size(); ++index)
            {
                const auto power = meterSamples[index] * meterSamples[index];
        ...
        if (processLoudnessSignal)
        {
            for (int channel = 0; channel < channelCount; ++channel)
            {
                const auto input = meterSamples[static_cast<size_t>(channel)];
            ...
            // BS.1770 loudness sums the weighted power of all programme channels.
            ...
            pendingLoudnessFrameEnergy += weightedSampleEnergy;
    ...
    if (processPeakSignal)
    {
        ...
        const auto peakHoldSamples = static_cast<uint64_t>(std::max(
            0.0, std::round(sampleRate * juce::jlimit(0.0f, 10000.0f, peakHoldMilliseconds) * 0.001)));
        ...
        for (size_t channel = 0; channel < blockPeaks.size(); ++channel)
        {
            ...
            if (options.peakRms)
            {
                ...
                // Insight uses the AES-17 RMS reference: a full-scale sine reads 0 dB.
                const auto rmsDecibels = juce::jmax(floorDecibels,
    ...
    if (options.loudness)
    {
        ...
        if (loudnessPowerWriteCount >= shortTermWindowSamples)
            publishedShortTermMaximumLufs.store(std::max(
                ...
                std::memory_order_release);
        if (integratedLufs > floorDecibels)
...
MeterProcessor::Values MeterProcessor::getValues() const noexcept
{
    Values values;
...
void MeterProcessor::copyHistory(const size_t series, std::vector<float>& destination) const
{
...
uint64_t MeterProcessor::getRevision() const noexcept
{
    return revision.load(std::memory_order_acquire);
}

void MeterProcessor::configureKWeighting() noexcept
{
    for (auto& filter : highShelves)
...
void MeterProcessor::updateRollingLoudnessPowers() noexcept
{
...
void MeterProcessor::publishLoudnessFrame(const bool includeLoudness,
    ...
    if (includeHistory)
    {
        publishHistoryValues({
            loudnessPowerWriteCount >= momentaryWindowSamples
                ? lufsFromPower(momentaryLufsPower) : floorDecibels,
            loudnessPowerWriteCount >= shortTermWindowSamples
                ? lufsFromPower(shortTermLufsPower) : floorDecibels,
            getIntegratedLufs()
        });
    }
...
void MeterProcessor::addLoudnessBlock(
    const float power,
    ...
    std::array<uint64_t, integratedLoudnessBinCount>& counts) noexcept
{
    ...
    const auto index = juce::jlimit(0, static_cast<int>(integratedLoudnessBinCount) - 1,
        juce::roundToInt((loudness - integratedLoudnessBinLow) / integratedLoudnessBinStep));
    energies[static_cast<size_t>(index)] += power;
    ++counts[static_cast<size_t>(index)];
}

float MeterProcessor::getIntegratedLufs() const noexcept
{
    auto energy = 0.0;
    uint64_t count = 0;
    for (size_t index = 0; index < integratedLoudnessBinCount; ++index)
    {
    ...
    const auto firstIndex = juce::jlimit(0, static_cast<int>(integratedLoudnessBinCount) - 1,
        static_cast<int>(std::ceil((relativeGate - integratedLoudnessBinLow) / integratedLoudnessBinStep)));
    energy = 0.0;
    count = 0;
    for (auto index = firstIndex; index < static_cast<int>(integratedLoudnessBinCount); ++index)
    {
...
float MeterProcessor::getLoudnessRange() const noexcept
{
    auto energy = 0.0;
    uint64_t count = 0;
    for (size_t index = 0; index < integratedLoudnessBinCount; ++index)
    ...
    const auto percentileIndex = [&] (const double percentile)
    {
        ...
        return static_cast<int>(integratedLoudnessBinCount) - 1;
    };

    const auto lowIndex = percentileIndex(0.10);
    const auto highIndex = percentileIndex(0.95);
    return std::max(0.0f, static_cast<float>(highIndex - lowIndex) * integratedLoudnessBinStep);
}

void MeterProcessor::publishHistoryValues(
